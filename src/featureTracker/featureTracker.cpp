...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time) {
    ...
    if (bflowBack_ && forw_pts.size() > 0) {
        vector<uchar> status;
        vector<float> err;
        std::vector<cv::Point2f> cur_pts_inverse = cur_pts;
        cv::calcOpticalFlowPyrLK(forw_img, cur_img, forw_pts, cur_pts_inverse, status, err, cv::Size(21, 21), 3);

        for (int i = 0; i < int(cur_pts_inverse.size()); i++) {
            if (status[i] && !inBorder(cur_pts_inverse[i])) {
                status[i] = 0;
                continue;
            }

            float dis = cv::norm(cur_pts_inverse[i] - cur_pts[i]);
            if (dis > 1) {
                status[i] = 0;
            }
        }
    ...
    if (PUB_THIS_FRAME) {
        ...
        if (n_max_cnt > 0) {
            ...
            // LOG(INFO) << "track points costs: " << t_track.toc();
        } else
            n_pts.clear();
...
void FeatureTracker::rejectWithF() {
    if (forw_pts.size() >= 8) {
        ...
        reduceVector(ids, status);
        reduceVector(track_cnt, status);
        // LOG(INFO) << "FM ransac costs: " <<  t_rejectF.toc();
        // printf("cur time: %f, pre time: %f, FM ransac: %d -> %lu: %f\n",cur_time, prev_time, size_a, forw_pts.size(), 1.0 * forw_pts.size() / size_a);
